...
async function add_value_col(country_id, datess, file_num) {
  const result = await sequelize
    .query(
      "select * from covid_infos where country_code=:value and dates=:val",
      {
        replacements: { value: country_id, val: datess },
      }
    )
    .then(async (data) => {
      const res =
...
async function main() {
  await update_cols(
    "/Users/bhagyalakshmi/Documents/Everest_Assignments/covid19/src/data/csv_files/recovered.csv",
    1
  );
  await update_cols(
    "/Users/bhagyalakshmi/Documents/Everest_Assignments/covid19/src/data/csv_files/deaths.csv",
    2
  );
}
...
module.exports = { update_cols, update_data, main };
